...
export default function Header() {
  return (
    <nav className="fixed top-0 inset-x-0 z-50 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 glass-card">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="text-xl font-black tracking-tight text-slate-900">
            Ismam<span className="text-indigo-600">.AI</span>
          </span>
        </Link>
        <div className="flex items-center gap-4">
          ...
          <SignedIn>
            <Link href="/generate" className="hidden md:flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-full text-sm font-bold hover:bg-indigo-600 hover:shadow-lg transition-all active:scale-95">
              Start Writing <span className="text-indigo-300">→</span>
            </Link>
