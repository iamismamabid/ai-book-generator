...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 overflow-hidden relative">
      ...
      <section id="testimonials" className="relative z-10 max-w-7xl mx-auto px-6 py-20 border-t border-slate-900/50">
        <div className="text-center mb-16">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-black uppercase tracking-wider mb-3">
            ★ Real Reviews. Genuine Traction
          </span>
          <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight">
            Trusted by KDP Self-Publishers
          </h2>
          <p className="text-slate-400 text-sm font-semibold max-w-md mx-auto mt-3">
            Here's what active publishers are saying about how Ismam Studio speeds up their KDP book launches.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            {
              quote: "The shaped maze engine is an absolute game changer. I generated 10 heart-shaped maze interiors with a single-solution guarantee in under 5 minutes and uploaded directly to KDP. No margin warnings!",
              author: "Sarah J.",
              role: "Indie Publisher (20+ books)",
              rating: 5,
              badge: "Maze Specialist"
            },
            {
              quote: "I used to spend hours manually calculating cover margins, spines, and gutter bleed guides. Ismam Studio automatically computes safety guides based on my page counts. Highly recommended!",
              author: "Marcus K.",
              role: "Low-Content Creator",
              rating: 5,
              badge: "KDP Master"
            },
            {
              quote: "Unlike other AI wrappers, Ismam Studio has a fully functional local puzzle compiler. The Sudoku and Word Search layouts compile mathematically unique grids every single run.",
              author: "Elena R.",
              role: "Children's Workbook Author",
              rating: 5,
              badge: "Puzzle Publisher"
            }
          ].map((t, idx) => (
            <div key={idx} className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/80 rounded-[2rem] p-8 flex flex-col justify-between space-y-6 relative hover:border-slate-750 transition-all duration-300">
              <div className="space-y-4">
                {/* Rating stars */}
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-800/60">
                <div>
                  ...
                  <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{t.role}</div>
                </div>
                <span className="text-[9px] font-black bg-indigo-500/10 text-indigo-400 px-2.5 py-1 rounded-full border border-indigo-500/20 uppercase">
                  {t.badge}
                </span>
              </div>
            </div>
