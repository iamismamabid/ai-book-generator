...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 overflow-hidden relative" suppressHydrationWarning>
      ...
      <section id="social-proof" className="relative z-10 max-w-7xl mx-auto px-6 py-20 border-t border-slate-900/50">
        {/* Glow highlights */}
        ...
        <div className="text-center mb-16">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs font-black uppercase tracking-wider mb-3">
            <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400 animate-pulse" /> Loved By Publishers
          </span>
          <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight font-sans">
            Why KDP Creators <span className="bg-gradient-to-r from-amber-300 via-rose-400 to-sky-300 bg-clip-text text-transparent">Choose Ismam Studio</span>
          </h2>
          <p className="text-slate-400 text-sm md:text-base max-w-xl mx-auto mt-3 font-semibold font-sans">
            Hear from self-publishers and designers who are building profitable books and scaling their KDP stores with our toolkit.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16 max-w-5xl mx-auto">
          {[
            { value: "10,000+", label: "Books Created" },
            { value: "250,000+", label: "Puzzles Generated" },
            { value: "4.9/5", label: "Average Rating" },
            { value: "100%", label: "KDP Bleed Compliant" }
          ].map((stat, idx) => (
            <div key={idx} className="bg-slate-900/25 border border-slate-900 rounded-3xl p-6 text-center backdrop-blur-sm">
              <div className="text-3xl md:text-4xl font-black text-white bg-gradient-to-r from-amber-200 to-rose-300 bg-clip-text text-transparent font-sans">
                {stat.value}
              </div>
              <div className="text-slate-400 text-xs font-bold uppercase tracking-wider mt-2 font-sans">
                {stat.label}
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            {
              quote: "I compiled 4 activity books in under a week using the Puzzle Generator. The margin guidelines and gutters are 100% accurate, saving me days of tedious layout formatting.",
              author: "Sarah Jenkins",
              role: "KDP Publisher (60+ Books)",
              rating: 5,
              avatar: "SJ"
            },
            {
              quote: "The Fabric Cover Studio is a game-changer. Being able to calculate and visualize the exact spine fold width in real time saved my covers from multiple KDP upload rejections.",
              author: "David Chen",
              role: "Full-Time Low-Content Creator",
              rating: 5,
              avatar: "DC"
            },
            {
              quote: "My kids coloring book was approved on the first try. The margin safety zone checker is extremely accurate and fast. Worth every penny of the lifetime deal!",
              author: "Elena Rostova",
              role: "Children's Activity Book Author",
              rating: 5,
              avatar: "ER"
            }
          ].map((item, idx) => (
            <div key={idx} className="bg-gradient-to-br from-slate-950/60 to-slate-900/40 border border-slate-900 p-8 rounded-[2.5rem] flex flex-col justify-between hover:border-slate-800 transition-all duration-300 group">
              <div>
                {/* Stars */}
                ...
                <p className="text-slate-350 text-sm md:text-base font-medium leading-relaxed mb-8 italic font-sans">
                  "{item.quote}"
                </p>
              </div>

              <div className="flex items-center gap-3 border-t border-slate-900/60 pt-6">
                ...
                <div>
                  <h4 className="text-sm font-black text-white font-sans">{item.author}</h4>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest font-sans">{item.role}</p>
                </div>
              </div>
            </div>
          ))}
