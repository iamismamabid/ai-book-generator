      ...
      <div className="fixed inset-0 z-[99999] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl border border-slate-200 max-w-md w-full shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
          <button
            ...
            className="absolute top-4 right-4 p-1.5 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-655 transition cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 p-3.5 bg-slate-50 rounded-2xl border border-slate-200/50">
              ...
              {includeCover && (
                <div className="flex gap-2 items-start bg-indigo-50 border border-indigo-200 text-indigo-855 p-2.5 rounded-xl text-[10px] font-semibold leading-normal mt-2">
            ...
            <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200/50 flex justify-between items-center">
              <div>
                <label className="text-xs font-black text-slate-800 block">KDP Book Trim Size</label>
                <span className="text-[9px] font-bold text-slate-400 block uppercase">Target paperback print size</span>
              </div>
              <select
                ...
                className="bg-white border border-slate-200 rounded-xl px-2 py-1 text-xs font-bold text-slate-700 outline-none"
              >
            ...
            {errors.length > 0 && (
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200/50 space-y-2 max-h-[150px] overflow-y-auto custom-scrollbar">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">KDP Requirements Checklist</span>
                  ...
                  {errors.map((err, idx) => (
                    <div
                      ...
                      className={`flex gap-2 items-center p-2 rounded-lg text-[9px] font-semibold ${
                        err.type === 'error'
                          ? 'bg-rose-50/50 border border-rose-100 text-rose-700'
          ...
          <div className="mt-6 flex gap-3">
            <button
              onClick={() => setIsExportModalOpen(false)}
              className="flex-1 py-3 border border-slate-200 hover:bg-slate-55 text-slate-700 rounded-xl text-xs font-black transition cursor-pointer"
            >
              CANCEL
            </button>
            <button
              ...
              className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black shadow-lg shadow-indigo-600/25 transition active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
            >
              {isExporting ? (
                <>
