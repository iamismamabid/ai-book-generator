...
export default function ByokEarlyLaunchModal({ isOpen, onClose, studioType }: ByokEarlyLaunchModalProps) {
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-gradient-to-r from-amber-500/20 via-orange-500/20 to-amber-500/20 border border-amber-500/40 text-amber-300 shadow-sm shadow-amber-500/10">
            <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-pulse" /> Early Launch News • BYOK System
          </span>
        ...
        <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-white mb-2 leading-tight">
          Bring Your Own Key <span className="bg-gradient-to-r from-amber-400 via-orange-400 to-amber-300 bg-clip-text text-transparent">(BYOK)</span> System Is Launching Soon!
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          ...
          <div className="p-4 rounded-2xl bg-slate-800/40 border border-slate-800/80 flex flex-col gap-2">
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Your API keys are encrypted and stored locally in your browser. They are never sent to or logged on KDPage servers.
            </p>
          </div>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2.5">
            ...
            <span className="text-[10px] text-amber-400 font-bold">Coming in v2.4</span>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-indigo-500/10 border border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-black text-white uppercase tracking-wider">
                {isSubscribed ? "You're on the Priority Launch List!" : "Get Early Access Launch Updates"}
              </h4>
              <p className="text-[11px] text-slate-300">
                {isSubscribed
                  ? "We'll enable the BYOK API Key panel in your studio the moment it goes live."
                  : "Click below to secure priority access when BYOK rolls out to this studio."}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {isSubscribed ? (
              ...
              >
                <Bell className="w-3.5 h-3.5" /> Notify Me On Launch
              </button>
            )}
          </div>
        </div>
