...
export default function ByokStudioPanel({
  ...
  // 1. Initial mount: Check server env key & determine best activeProvider
  useEffect(() => {
    fetch("/api/byok/generate")
      .then((res) => res.json())
      .then((data) => {
        if (data?.hasServerGeminiKey) {
          setServerGeminiAvailable(true);
          setServerGeminiHint(data.geminiKeyHint || null);
        }
      })
      .catch(() => {});
    ...
    // Auto-detect which provider should be selected:
    // Priority: Saved provider in localStorage -> Any provider with saved key -> Gemini
    const savedProvider = loadActiveProvider();
    let initialProvider: ByokProvider = "gemini";

    if (savedProvider && (loaded[savedProvider] || (!loaded.openai && !loaded.gemini && !loaded.stability))) {
      initialProvider = savedProvider;
    } else if (loaded.gemini && loaded.gemini.length > 5) {
      initialProvider = "gemini";
    } else if (loaded.openai && loaded.openai.length > 5) {
      initialProvider = "openai";
    } else if (loaded.stability && loaded.stability.length > 5) {
      initialProvider = "stability";
  ...
  // 2. Sync keys from localStorage on activeProvider switch
  useEffect(() => {
    const loaded = loadByokKeys();
    setKeys(loaded);
    ...
    const hasKey = Boolean(existing && (existing.length > 5 || existing === "env"));
    setIsKeySaved(hasKey);
    setIsEditingKey(!hasKey);
    setErrorMsg(null);
  }, [activeProvider]);

  const handleSelectProvider = (p: ByokProvider) => {
    // Auto-save if user entered a key before switching tabs
    if (currentKeyInput.trim().length > 5 && currentKeyInput.trim() !== keys[activeProvider]) {
      const updated = saveByokKey(activeProvider, currentKeyInput.trim());
      setKeys(updated);
    }
    setActiveProvider(p);
  ...
  const handleGenerate = async () => {
    let activeKey = keys[activeProvider] || currentKeyInput.trim();

    // Auto-save key if user entered it in input but didn't explicitly click "Save Key"
    if (currentKeyInput.trim().length > 5 && currentKeyInput.trim() !== keys[activeProvider]) {
      const updated = saveByokKey(activeProvider, currentKeyInput.trim());
      setKeys(updated);
      setIsKeySaved(true);
      setIsEditingKey(false);
      activeKey = currentKeyInput.trim();
    }

    // Support platform / .env key for Gemini if available
    if ((!activeKey || activeKey.length < 5) && activeProvider === "gemini" && serverGeminiAvailable) {
      activeKey = "env";
    }
  ...
  return (
    <div className="space-y-4 text-slate-800 dark:text-slate-200">
      ...
      <div className="flex items-center justify-between">
        ...
        ) : activeProvider === "gemini" && serverGeminiAvailable ? (
          <span className="text-[11px] font-bold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/50 px-2.5 py-1 rounded-lg border border-blue-200 dark:border-blue-800 flex items-center gap-1">
            <Sparkles className="w-3.5 h-3.5 text-blue-600" /> Platform Key
          </span>
        ) : (
      ...
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-3 gap-1.5 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl">
          {(["openai", "gemini", "stability"] as ByokProvider[]).map((p) => {
            const hasKey = Boolean(keys[p] && (keys[p].length > 5 || keys[p] === "env"));
            const isEnvActive = p === "gemini" && serverGeminiAvailable && !hasKey;
            ...
            return (
              ...
              >
                ...
                {hasKey && <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" title="Key Configured & Saved" />}
                {isEnvActive && <span className="w-2 h-2 rounded-full bg-blue-500 shrink-0" title="Environment Key Available" />}
              </button>
            );
          })}
        </div>
      ...
      {/* 3. API Key Card */}
      {!isEditingKey && (isKeySaved || (activeProvider === "gemini" && serverGeminiAvailable && currentKeyInput === "env")) ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                  {maskApiKey(keys[activeProvider] || (serverGeminiAvailable ? "env" : ""))}
                </span>
              </div>
            </div>
        ...
        <div className="p-3.5 bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2.5 shadow-xs">
          {/* Optional .env Key Quick Action */}
          {activeProvider === "gemini" && serverGeminiAvailable && (
            <div className="flex items-center justify-between p-2 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/60 text-xs">
              <div className="flex items-center gap-1.5 text-blue-800 dark:text-blue-300 text-[11px] font-medium">
                <Sparkles className="w-3.5 h-3.5 text-blue-500 shrink-0" />
                <span>Pre-configured key in .env ({serverGeminiHint || "Available"})</span>
              </div>
              <button
                type="button"
                onClick={() => {
                  ...
                  setKeys(updated);
                  setCurrentKeyInput("env");
                  setIsKeySaved(true);
                  setIsEditingKey(false);
                  setActionFeedback("Platform Gemini key activated!");
                  setTimeout(() => setActionFeedback(null), 3000);
                }}
                className="px-2.5 py-1 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold text-[11px] cursor-pointer shadow-xs transition"
              >
                Use .env Key
              </button>
            </div>
