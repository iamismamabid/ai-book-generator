...
export default class CoverStudioErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    if (isChunkError && typeof window !== "undefined") {
      const lockKey = "kdpage_chunk_reload_cover";
    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-[#0b0f19] text-white select-none">
          <div className="max-w-md w-full bg-slate-900/80 border border-indigo-500/30 rounded-3xl p-8 text-center space-y-5 shadow-2xl backdrop-blur-xl">
            ...
            <div className="space-y-2">
              <h2 className="text-xl font-black text-white tracking-tight">
                {isChunkError ? "New Version Deployed" : (this.props.title || "Cover Studio Canvas Paused")}
              </h2>
              <p className="text-slate-400 text-xs font-semibold leading-relaxed">
                {isChunkError
                  ? "A new update of KDPage was just released. Your saved design elements and settings are safely stored in your browser."
            ...
            <div className="pt-2 flex flex-col gap-2">
              ...
              >
                <RefreshCw className="w-4 h-4" /> {isChunkError ? "Update & Reload Studio" : "Reload Studio Canvas"}
              </button>
              {!isChunkError && (
                <button
                  onClick={this.handleClearCorruptCacheAndReload}
                  className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 font-semibold text-xs rounded-xl transition-all duration-200 cursor-pointer"
                >
