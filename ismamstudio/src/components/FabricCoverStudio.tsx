...
export default function FabricCoverStudio({
  ...
  const updateActiveObjectProperty = (property: string, value: any, saveHistory = true) => {
    ...
    if (hasSelection && textObj && perCharProperties.includes(property)) {
      ...
      if (property === 'flipX' || property === 'flipY') {
        ...
        if (textObj && perCharProperties.includes(property)) {
          // If changing a text property on the entire text object without selection,
          // clear individual char overrides so the whole text gets the new styling consistently
          const styles = (textObj as any).styles;
          if (styles) {
            for (const line in styles) {
              for (const char in styles[line]) {
                if (styles[line][char] && styles[line][char][property] !== undefined) {
                  delete styles[line][char][property];
                }
              }
            }
          }
  ...
  const addAuthorText = () => {
    if (!canvas) return;
    const author = new fabric.IText("By Author Name", {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden h-full relative">
      ...
      {activeToolTab && (
        <div className={`${activeToolTab === 'ai-byok' ? 'w-88 sm:w-96' : 'w-80'} bg-slate-50 border-r border-slate-200 flex flex-col p-5 overflow-y-auto shrink-0 absolute md:relative left-16 top-0 h-full z-30 shadow-2xl md:shadow-none transition-all duration-150`}>
          ...
          <div className="space-y-4">
            ...
            <div className="flex bg-slate-200/60 p-0.5 rounded-lg border border-slate-300/40 text-[10px] font-black uppercase">
              <button 
                ...
                className={`flex-1 py-1 rounded-md text-center transition-all cursor-pointer ${graphicsSubTab === 'kdp-icons' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500 hover:text-slate-900'}`}
              >
                KDP Icons
              </button>
              <button 
                onClick={() => setGraphicsSubTab('unsplash')}
                className={`flex-1 py-1 rounded-md text-center transition-all cursor-pointer ${graphicsSubTab === 'unsplash' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500 hover:text-slate-900'}`}
              >
                Search Photos
              </button>
            </div>
