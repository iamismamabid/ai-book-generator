/**
 * KDPage Slitherlink PDF Exporter
...
export async function exportSlitherlinkBookPdf(
  puzzles: SlitherlinkPuzzle[],
  options: SlitherlinkPdfOptions,
  ...
  doc.text(
    options.bookTitle || "SLITHERLINK PUZZLE BOOK",
    pageWidth / 2,
    pageHeight * 0.38,
    { align: "center" }
  );
  ...
  doc.text(
    options.bookSubtitle || "Classic Fences & Loop-the-Loop Logic Puzzles",
    pageWidth / 2,
    pageHeight * 0.44,
    { align: "center" }
  );
  ...
  if (options.includeSolutions && puzzles.length > 0) {
    ...
    doc.setFontSize(26);
    doc.setTextColor(30, 41, 59);
    doc.text("SOLUTIONS & LOOPS", pageWidth / 2, pageHeight * 0.45, {
      align: "center",
    });
...
export function drawSlitherlinkPuzzle(
  doc: jsPDF,
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(Math.max(8, Math.round(cellSize * 0.44)));
  doc.setTextColor(isSolution ? 140 : 30, isSolution ? 140 : 41, isSolution ? 140 : 59);
  ...
  for (let r = 0; r < puzzle.rows; r++) {
    for (let c = 0; c < puzzle.cols; c++) {
      ...
      if (clue !== null && clue !== undefined) {
        ...
        const cellCenterY = startY + r * cellSize + cellSize / 2 + cellSize * 0.15;
  ...
  if (isSolution) {
    doc.setDrawColor(15, 23, 42); // bold dark loop line
    doc.setLineWidth(Math.max(1.8, cellSize * 0.08));

    // Horizontal solution lines
    for (let r = 0; r <= puzzle.rows; r++) {
      for (let c = 0; c < puzzle.cols; c++) {
  ...
  // Draw Dot Grid (at all (r, c) vertices)
  const dotRadius = Math.max(1.4, Math.min(2.5, cellSize * 0.065));
